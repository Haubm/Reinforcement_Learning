"""
Learning from Youtube: "Machine Learning with Phil"
"""
import numpy as np
...
class GridWorld(object):
    def __init__(self, m, n, magicSquares):
        ...
        # The state space don't include the terminate state
    ...
    def isTerminateState(self, state):
    ...
    def step(self, action):
        ...
        reward = -1 if not self.isTerminateState(resultingState) else 0
        if not self.offGridMove(resultingState, self.agentPosition):
            self.setState(resultingState)
            return resultingState, reward, \
                    self.isTerminateState(self.agentPosition), None
        else:
...
if __name__ == '__main__':
    magicSquare = {18: 54, 63: 14}
    env = GridWorld(9,9, magicSquare)
    ...
    for i in range(numGames):
        if i % 5000 == 0:
            print('starting game', i,'\t point:', epRewards)
